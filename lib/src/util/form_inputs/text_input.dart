...
import 'package:hostapp/src/util/form_inputs/input_validator.dart';
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Stack(
        ...
        children: <Widget>[
          if (widget.label.isNotEmpty) Positioned(top: -24, child: Text(widget.label, style: AppTextStyle.inputLabel)),
          TextFormField(
            initialValue: _getInitialValue(),
            style: AppTextStyle.orderTotalLabel,
            enabled: widget.isActive,
            onChanged: _handleChange,
            keyboardType: _setKeyboardType(),
            autovalidate: _isAutoValidating,
            validator: _validateField,
            decoration: AppTextStyle.getInputDecoration(helper: widget.helper),
          ),
          Positioned(
            top: 6,
            left: 12,
            child: Text(_getLabel().toUpperCase(), style: AppTextStyle.labelOptional),
          ),
          if (_errorText.isNotEmpty)
            Positioned(
              top: 8,
              right: 14,
              child: Text(_errorText.toUpperCase(), style: AppTextStyle.labelNotValid),
            ),
        ],
      ),
    );
